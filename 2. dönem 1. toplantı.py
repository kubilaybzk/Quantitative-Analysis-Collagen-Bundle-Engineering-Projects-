...
for i in range(0,512):
    for k in range(0,512):
        if(0<=colAssignment[i][k]<2):
            colAssignment[i][k]=1
        if (2 <= colAssignment[i][k]<4):
            colAssignment[i][k] =3
        if (4 <= colAssignment[i][k]<6):
            colAssignment[i][k] =5
        if (6 <= colAssignment[i][k]<8):
            colAssignment[i][k] =7
        if (8 <= colAssignment[i][k]<=9):
            colAssignment[i][k] =9
...
plt.imsave("nothasta_Max_Responce.png", selVals, cmap='gray')
...
im = cv2.imread("saglikli3.png", 0)  # dosyayi oku
plt.subplot(2, 2, 1)
